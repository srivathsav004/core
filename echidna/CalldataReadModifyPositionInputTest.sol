...
contract CalldataReadModifyPositionInputTest {
    ...
    function echidna_test_readModifyPositionInput() public {
        ...
        // If successful, verify memory contents are reasonable
        if (success) {
            foundValidInput = true;
            
            // Basic sanity checks on memory layout
            // The function should not revert with valid-ish inputs
            assert(true);
        } else {
            // Check if it failed for expected reasons
            // We can't easily check the revert reason in Echidna, but we can track
            // that we're getting failures
